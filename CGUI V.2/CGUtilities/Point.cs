...
namespace CGUtilities
{
    ...
    public class Point : ICloneable
    {
        ...
        public Point Prev(List<Point> polyPoints)
        {
            int index = polyPoints.IndexOf(this) - 1;
            if (index < 0)
                return polyPoints[polyPoints.Count - 1];
            else
                return polyPoints[index];
            
        }

        public Point Next(List<Point> polyPoints)
        {
            int index = polyPoints.IndexOf(this) + 1;
            if (index > polyPoints.Count - 1)
                return polyPoints[0];
            else
                return polyPoints[index];
        }
